...
const Chapter2 = () => {
  ...
  const nextSection = () => {
    if (currentSection < chapter.subsections.length - 1) {
      ...
      navigate("/van-de-dan-toc/chuong-3");
    }
  };
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl relative">
        ...
        <div className="mb-12 text-center">
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-ethnic-green to-ethnic-emerald rounded-full mb-4 shadow-2xl"
          >
            <span className="text-4xl">{chapter.emoji}</span>
          </motion.div>

          <div className="text-sm font-bold text-ethnic-green mb-2">
            {chapter.number} / 4
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="bg-yellow-900/30 backdrop-blur-md border border-yellow-600/40 rounded-3xl p-8 md:p-12 shadow-xl mb-8">
              ...
              {/* Challenges (Chapter 2.3) */}
              {currentSubsection.content.challenges && (
                <div className="space-y-4 mb-8">
                  {currentSubsection.content.challenges.map((challenge, index) => (
                    <div
                      key={index}
                      className="p-6 bg-red-900/20 border border-red-600/30 rounded-2xl"
                    >
                      <div className="flex items-start gap-4">
                        <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-br from-red-500 to-red-700 rounded-full flex items-center justify-center font-black text-white">
                          {index + 1}
                        </div>
                        <div className="flex-1">
              ...
              {/* Causes (Chapter 2.4) */}
              {currentSubsection.content.causes && (
                <div className="mb-8">
                  <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                    <AlertTriangle className="w-6 h-6 text-vietnam-yellow" />
                    Nguyên nhân
                  </h3>
                  <div className="grid md:grid-cols-2 gap-6">
                    {currentSubsection.content.causes.map((cause, index) => (
                      <div
                        key={index}
                        className="p-6 bg-ethnic-purple/10 border border-ethnic-purple/30 rounded-2xl"
                      >
                        <h4 className="text-lg font-bold text-ethnic-purple mb-3">
                          {cause.type}
                        </h4>
                        <ul className="space-y-2">
                          {cause.items.map((item, itemIndex) => (
                            <li key={itemIndex} className="flex items-start gap-2">
                              <span className="text-ethnic-purple font-bold">•</span>
              ...
              {currentSubsection.content.solutions && (
                <div className="mb-8">
                  <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                    <Lightbulb className="w-6 h-6 text-vietnam-yellow" />
                    Định hướng giải pháp
                  </h3>
                  <div className="space-y-4">
                    {currentSubsection.content.solutions.map((solution, index) => (
                      <div
                        key={index}
                        className="flex gap-4 p-5 bg-ethnic-blue/10 border border-ethnic-blue/30 rounded-xl"
                      >
                        <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-ethnic-blue to-ethnic-cyan rounded-full flex items-center justify-center font-bold text-white text-sm">
                          {index + 1}
                        </div>
                        <div className="flex-1">
                          <h4 className="font-bold text-white mb-2">{solution.title}</h4>
                          <p className="text-slate-300 text-sm leading-relaxed">{solution.content}</p>
                        </div>
                      </div>
                    ))}
                  </div>
