...
const Chapter1 = () => {
  ...
  const nextSection = () => {
    if (currentSection < chapter.subsections.length - 1) {
      ...
      // Last section - complete chapter
      saveProgress(chapter.id, currentSubsection.id);
      completeChapter(chapter.id);
      // Navigate to next chapter or back to hub
      navigate("/van-de-dan-toc/chuong-2");
    }
  };
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl relative">
        ...
        <div className="mb-12 text-center">
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-ethnic-blue to-ethnic-cyan rounded-full mb-4 shadow-2xl"
          >
            <span className="text-4xl">{chapter.emoji}</span>
          </motion.div>

          <div className="text-sm font-bold text-ethnic-blue mb-2">
            {chapter.number} / 4
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="bg-yellow-900/30 backdrop-blur-md border border-yellow-600/40 rounded-3xl p-8 md:p-12 shadow-xl mb-8">
              ...
              {/* Principles if exists */}
              {currentSubsection.content.principles && (
                <div className="space-y-4 mb-8">
                  {currentSubsection.content.principles.map((principle, index) => (
                    <div
                      key={index}
                      className="flex gap-4 p-4 bg-ethnic-blue/10 border border-ethnic-blue/30 rounded-xl"
                    >
                      <CheckCircle className="w-6 h-6 text-ethnic-blue flex-shrink-0 mt-1" />
                      <div>
                        <h4 className="font-bold text-white mb-2">{principle.title}</h4>
                        <p className="text-slate-300">{principle.content}</p>
                      </div>
                    </div>
                  ))}
                </div>
