...
const Chapter4 = () => {
  ...
  const nextSection = () => {
    if (currentSection < chapter.subsections.length - 1) {
      ...
      navigate("/van-de-dan-toc/hoan-thanh");
    }
  };
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl relative">
        ...
        <div className="mb-12 text-center">
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-vietnam-yellow to-ethnic-orange rounded-full mb-4 shadow-2xl"
          >
            <span className="text-4xl">{chapter.emoji}</span>
          </motion.div>

          <div className="text-sm font-bold text-vietnam-yellow mb-2">
            {chapter.number} / 4
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="bg-yellow-900/30 backdrop-blur-md border border-yellow-600/40 rounded-3xl p-8 md:p-12 shadow-xl mb-8">
              ...
              {/* Note if exists (Chapter 4.3) */}
              {currentSubsection.content.note && (
                <div className="mb-8 p-5 bg-gradient-to-r from-vietnam-yellow/10 to-ethnic-orange/10 border-l-4 border-vietnam-yellow rounded-r-2xl">
              ...
              {/* Conclusions (Chapter 4.1) */}
              {currentSubsection.content.conclusions && (
                <div className="space-y-6 mb-8">
                  {currentSubsection.content.conclusions.map((conclusion, index) => (
                    <div
                      key={index}
                      className={`p-6 rounded-2xl ${
                        conclusion.highlight
                          ? "bg-gradient-to-r from-vietnam-red/20 to-vietnam-yellow/20 border-2 border-vietnam-yellow"
                          : "bg-yellow-800/20 border border-yellow-600/30"
                      }`}
                    >
                      <div className="flex items-start gap-4">
                        {conclusion.highlight && (
                          <Flag className="w-7 h-7 text-vietnam-yellow flex-shrink-0 mt-1" />
                        )}
                        <div className="flex-1">
                          <h3 className={`text-xl font-bold mb-3 ${
                            conclusion.highlight ? "text-vietnam-yellow" : "text-white"
                          }`}>
                            {conclusion.title}
                          </h3>
              ...
              {/* Lessons (Chapter 4.2) */}
              {currentSubsection.content.lessons && (
                <div className="space-y-6 mb-8">
                  <div className="flex items-center gap-3 mb-6">
                    <Award className="w-7 h-7 text-vietnam-yellow" />
                  ...
                  {/* Final Quote */}
                  {currentSubsection.content.finalQuote && (
                    <div className="mt-8 p-6 bg-gradient-to-r from-vietnam-red/20 to-vietnam-yellow/20 border-l-4 border-vietnam-yellow rounded-r-2xl">
                      <div className="flex items-start gap-4">
                        <BookOpen className="w-6 h-6 text-vietnam-yellow flex-shrink-0 mt-1" />
              ...
              {/* AI Purposes (Chapter 4.3) */}
              {currentSubsection.content.purposes && (
                <div className="space-y-6 mb-8">
                  <div className="flex items-center gap-3 mb-6">
                    <Sparkles className="w-7 h-7 text-vietnam-yellow" />
                    <h3 className="text-2xl font-bold text-white">
                      Vai trò của AI
                    </h3>
                  </div>
                  {currentSubsection.content.purposes.map((purpose, index) => (
                    <div
                      key={index}
                      className="p-6 bg-gradient-to-r from-ethnic-purple/10 to-ethnic-pink/10 border border-ethnic-purple/30 rounded-2xl"
                    >
                      <div className="flex gap-4">
                        <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-br from-ethnic-purple to-ethnic-pink rounded-full flex items-center justify-center font-black text-white text-lg">
                          {index + 1}
                        </div>
                        <div className="flex-1">
